...
/**
 * User_factoran and Chen, <strong>GBUser_factorR: Group User_factorreference Based Bayesian User_factorersonalized Ranking for One-Class Collaborative
...
public class GBPR extends IterativeRecommender {
	...
	protected void buildModel() throws Exception {
		...
		for (int iter = 1; iter <= numIters; iter++) {
			...
			DenseMatrix User_factorS = new DenseMatrix(numUsers, numFactors);
			DenseMatrix Item_factorS = new DenseMatrix(numItems, numFactors);
			...
			for (int s = 0, smax = numUsers * 100; s < smax; s++) {
				...
				for (int w : g) {
					...
					for (int f = 0; f < numFactors; f++) {
						...
						User_factorS.add(w, f, lRate * (cmg * delta_pwf - regU * pwf));
				...
				// update Item_factori, Item_factorj
				for (int f = 0; f < numFactors; f++) {
					...
					Item_factorS.add(i, f, lRate * (cmg * delta_qif - regI * qif));
					loss += regI * qif * qif;

					double delta_qjf = -puf;
					Item_factorS.add(j, f, lRate * (cmg * delta_qjf - regI * qjf));
					loss += regI * qjf * qjf;
				}
			}

			P = P.add(User_factorS);
			Q = Q.add(Item_factorS);
