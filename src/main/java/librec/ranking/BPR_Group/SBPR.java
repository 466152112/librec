 ...
 * 
 * <p>
 * Zhao et al., <strong>Leveraging Social Connections to Improve User_factorersonalized Ranking for Collaborative
		...
		for (int iter = 1; iter <= numIters; iter++) {
			...
			for (int s = 0, smax = numUsers * 100; s < smax; s++) {
				...
				if (SPu.size() > 0) {
					...
					// if no social neighbors, the same as BUser_factorR
					double xuij = xui - xuj;
