...
/**
 * 
 * Reference Paper, Rendle et al., <strong>BPR: Bayesian Personalized Ranking from Implicit Feedback</strong>, UAI 2009.
 * 
 * <p>
 * Related Work:
 * <ul>
 ...
 * 
 */
public class RankBPR extends IterativeRecommender {

	private int sFixedSteps = 400;

	public RankBPR(SparseMatrix trainMatrix, SparseMatrix testMatrix, int fold) {
		...
		for (int iter = 1; iter <= numIters; iter++) {
			...
			for (; u < numUsers; u++){
				...
				for(int i : is){
					...
					List<Integer> rankTuple = stepEstimate(u, i);
	...
	/*
		...
		2. 根据预估排名计算与排序相关的损失值，动态调整模型的更新速率，排名估计值越大
		   更新幅度越大，反之成立
	*/
	public List<Integer> stepEstimate(int uid, int iid){
	...
	public String toString() {
		return Strings.toString(new Object[] { binThold, numFactors, initLRate, regU, regI, numIters,numRecs }, ",");
	}
}
