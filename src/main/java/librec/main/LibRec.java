...
import librec.ranking.MMMF;
import librec.ranking.RankALS;
import librec.ranking.RankSGD;
import librec.ranking.SLIM;
import librec.ranking.WRMF;
import librec.ranking.BPR_Group.AdaBPR;
import librec.ranking.BPR_Group.BPR;
import librec.ranking.BPR_Group.GBPR;
import librec.ranking.BPR_Group.SBPR;
import librec.ranking.BPR_Group.StatAdaBPR;
import librec.ranking.BPR_Group.StatBPR;
import librec.ranking.BPR_Group.StatStaticBPR;
import librec.ranking.BPR_Group.StaticBPR;
import librec.ranking.IBPR.AdaIBPR;
import librec.ranking.IBPR.ChangeAdaIBPR;
import librec.ranking.IBPR.ChangeIBPR;
import librec.ranking.IBPR.IBPR;
import librec.ranking.IBPR.IBPRSim;
import librec.ranking.IBPR.RandomIBPR;
import librec.ranking.RFBPR.RFBPR;
import librec.ranking.rating.RankBasedSVD;
import librec.ranking.tag.CD_BPR;
import librec.ranking.tag.FolkRank;
import librec.ranking.temporal.TimeRanking;
import librec.ranking.yulu.MBPR;
import librec.ranking.yulu.RankBPR;
import librec.ranking.yulu.RankDBPR;
import librec.ranking.yulu.RankMBPR;
...
import librec.rating.RatingBPR;
...
public class LibRec {
	...
	public static void main(String[] args) throws Exception {
		...
		trainFile=new File(cf.getPath("dataset.training"));
		// debug info
		debugInfo();
	...
	private static Recommender getRecommender(SparseMatrix[] data, int fold) throws Exception {
		...
		/* baselines */
			...
			case "commonneighbors":
				return new Commonneighbors(trainMatrix, testMatrix, fold);
			...
			/*extension BPR */
			case "adabpr":
				return new AdaBPR(trainMatrix, testMatrix, fold);
			case "staticbpr":
				return new StaticBPR(trainMatrix, testMatrix, fold);
			case "statstaticbpr":
				return new StatStaticBPR(trainMatrix, testMatrix, fold);
			case "statadabpr":
				return new StatAdaBPR(trainMatrix, testMatrix, fold);
			case "statbpr":
				return new StatBPR(trainMatrix, testMatrix, fold);
				/*myself */
			case "ibpr":
				return new IBPR(trainMatrix, testMatrix, fold);
			case "adaibpr":
				return new AdaIBPR(trainMatrix, testMatrix, fold);
			case "randomibpr":
				return new RandomIBPR(trainMatrix, testMatrix, fold);
			case "mmmf":
				return new MMMF(trainMatrix, testMatrix, fold);
			case "ibprsim":
				return new IBPRSim(trainMatrix, testMatrix, fold);
			case "rfbpr":
				return new RFBPR(trainMatrix, testMatrix, fold);
			case "ratingbpr":
				return new RatingBPR(trainMatrix, testMatrix, fold);
			case "changeibpr":
				return new ChangeIBPR(trainMatrix, testMatrix, fold);
			case "changeadaibpr":
				return new ChangeAdaIBPR(trainMatrix, testMatrix, fold);
			case "rankbasedsvd":
				return new RankBasedSVD(trainMatrix, testMatrix, fold);
			
				/*yulu code*/
			case "rankbpr":
				return new RankBPR(trainMatrix, testMatrix, fold);
			case "rankmbpr":
				return new RankMBPR(trainMatrix, testMatrix, fold);
			case "mbpr":
				return new MBPR(trainMatrix, testMatrix, fold);
			case "rankdbpr":
				return new RankDBPR(trainMatrix, testMatrix, fold);
				
				/*tag recommender*/
			case "cd_bpr":
				return new CD_BPR(trainMatrix, testMatrix, fold);
			case "folkrank":
				return new FolkRank(trainMatrix, testMatrix, fold);
			/* temporal ranking
			 */
			case "timeranking":
				return new TimeRanking(trainMatrix, testMatrix, fold);
